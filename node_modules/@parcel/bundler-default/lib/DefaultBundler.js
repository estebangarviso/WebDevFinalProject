  ...
  bundle({
        ...
        if (resolution && (!bundleGroup || dependency.priority === 'lazy' || dependency.priority === 'parallel' || resolution.bundleBehavior === 'isolated' || resolution.bundleBehavior === 'inline')) {
          ...
          for (let asset of assets) {
            let bundle = bundleGraph.createBundle({
              entryAsset: asset,
              isEntry: asset.bundleBehavior === 'inline' ? false : dependency.isEntry || dependency.needsStableName,
        ...
        for (let asset of assets) {
          ...
          if (existingBundle) {
            ...
            let bundle = bundleGraph.createBundle({
              ...
              type: asset.type,
              target: bundleGroup.target,
              isEntry: asset.bundleBehavior === 'inline' || dependency.priority === 'parallel' && !dependency.needsStableName ? false : parentBundle.isEntry,
              isInline: asset.bundleBehavior === 'inline',
    ...
    bundleGraph.traverseBundles(bundle => {
      ...
      for (let candidate of candidates) {
        ...
        if (Array.from(bundleGroups).every(group => bundleGraph.getBundlesInBundleGroup(group).filter(b => !b.isInline).length < config.maxParallelRequests)) {
    ...
    deduplicate(bundleGraph); // Step 4: Mark async dependencies on assets that are already available in
  ...
  optimize({
    ...
    bundleGraph.traverse((node, ctx, actions) => {
      ...
      .filter(b => {
        ...
        return !b.isEntry && b.isSplittable && entries.every(entry => entry.id !== asset.id);
      });
    ...
    for (let {
      ...
      let sharedBundle = bundleGraph.createBundle({
        uniqueKey: (0, _hash().hashString)([...sourceBundles].map(b => b.id).join(':')),
      ...
      for (let asset of assets) {
        ...
        for (let bundle of sourceBundles) {
          ...
          if (bundleGroups.every(bundleGroup => bundleGraph.getBundlesInBundleGroup(bundleGroup).filter(b => !b.isInline).length < config.maxParallelRequests)) {
