...
const SCRIPT_ERRORS = {
  browser: 'Browser scripts cannot have imports or exports. Use a <script type="module"> instead.',
  'web-worker': 'Web workers cannot have imports or exports. Use the `type: "module"` option instead.',
  'service-worker': 'Service workers cannot have imports or exports. Use the `type: "module"` option instead.'
};
...
var _default = new (_plugin().Transformer)({
  ...
  async transform({
    ...
    if (script_error_loc) {
      let message = SCRIPT_ERRORS[asset.env.context];
    ...
    for (let dep of dependencies) {
      if (dep.kind === 'WebWorker') {
        ...
        if (asset.env.sourceType === 'script' && dep.source_type === 'Module' && supportsModuleWorkers) {
        ...
        if (asset.env.isWorker()) {
          if (asset.env.sourceType !== 'script') {
            ...
            let diagnostic = [{
              ...
              hints: ['Try using a static `import`, or dynamic `import()` instead.']
            }];
            throw new (_diagnostic().default)({
              diagnostic
            });
    ...
    if (hoist_result) {
      ...
      let deps = new Map(asset.getDependencies().map(dep => [dep.specifier, dep]));
