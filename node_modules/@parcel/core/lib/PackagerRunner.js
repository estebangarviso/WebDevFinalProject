...
class PackagerRunner {
  constructor({
    ...
    _defineProperty(this, "previousInvalidations", void 0);
  ...
  async package(internalBundle, bundleGraph, configs) {
    ...
    try {
      ...
      let devDepRequest = await (0, _DevDepRequest().createDevDependency)({
        specifier: name,
        resolveFrom
      }, packager, this.previousDevDeps, this.options);
  ...
  async optimize(internalBundle, internalBundleGraph, type, contents, map, configs) {
    ...
    for (let optimizer of optimizers) {
      try {
        ...
        }, optimizer, this.previousDevDeps, this.options);
        this.devDepRequests.set(`${optimizer.name}:${optimizer.resolveFrom}`, devDepRequest);
      }
    }
  ...
  async getCacheKey(bundle, bundleGraph, configs, invalidations) {
    ...
    return (0, _hash().hashString)(_constants().PARCEL_VERSION + devDepHashes + invalidationHash + bundle.target.publicUrl + bundleGraph.getHash(bundle) + JSON.stringify(configResults));
  }
