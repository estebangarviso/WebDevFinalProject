...
function _ContentGraph() {
  const data = _interopRequireDefault(require("./ContentGraph"));

  _ContentGraph = function () {
    return data;
  };
...
class BundleGraph {
  ...
  static fromAssetGraph(assetGraph, publicIdByAssetId = new Map(), assetPublicIds = new Set()) {
    let graph = new (_ContentGraph().default)();
  ...
  static deserialize(serialized) {
    return new BundleGraph({
      graph: _ContentGraph().default.deserialize(serialized.graph),
  ...
  addAssetGraphToBundle(asset, bundle, shouldSkipDependency = d => this.isDependencySkipped(d)) {
    ...
    this._graph.traverse((nodeId, _, actions) => {
      ...
      if (node.type === 'dependency') {
        ...
        if (this._graph.getNodeIdsConnectedFrom(nodeId, 'references').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).some(node => node.type === 'bundle')) {
  ...
  internalizeAsyncDependency(bundle, dependency) {
    ...
    this._graph.addEdge(this._graph.getNodeIdByContentKey(bundle.id), this._graph.getNodeIdByContentKey(dependency.id), 'internal_async');
  ...
  getParentBundlesOfBundleGroup(bundleGroup) {
    return this._graph.getNodeIdsConnectedTo(this._graph.getNodeIdByContentKey((0, _utils2().getBundleGroupId)(bundleGroup)), 'bundle').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).filter(node => node.type === 'bundle').map(node => {
  ...
  resolveAsyncDependency(dependency, bundle) {
    ...
    if (bundleNodeId != null && this._graph.hasEdge(bundleNodeId, depNodeId, 'internal_async')) {
      let referencedAssetNodeIds = this._graph.getNodeIdsConnectedFrom(depNodeId, 'references');
  ...
  getReferencedBundle(dependency, fromBundle) {
    ...
    let referenced = this._graph.getNodeIdsConnectedFrom(dependencyNodeId, 'references').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).find(node => node.type === 'asset');
  ...
  removeBundle(bundle) {
    ...
    let bundleGroupNodeIds = this._graph.getNodeIdsConnectedTo(bundleNodeId, 'bundle');
  ...
  removeExternalDependency(bundle, dependency) {
    ...
    for (let bundleGroupNode of this._graph.getNodeIdsConnectedFrom(this._graph.getNodeIdByContentKey(dependency.id)).map(id => (0, _nullthrows().default)(this._graph.getNode(id))).filter(node => node.type === 'bundle_group')) {
      ...
      if (inboundDependencies.every(dependency => !this.bundleHasDependency(bundle, dependency) || this._graph.hasEdge(bundleNodeId, this._graph.getNodeIdByContentKey(dependency.id), 'internal_async'))) {
  ...
  createBundleReference(from, to) {
    this._graph.addEdge(this._graph.getNodeIdByContentKey(from.id), this._graph.getNodeIdByContentKey(to.id), 'references');
  }

  findBundlesWithAsset(asset) {
    return this._graph.getNodeIdsConnectedTo(this._graph.getNodeIdByContentKey(asset.id), 'contains').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).filter(node => node.type === 'bundle').map(node => {
      (0, _assert().default)(node.type === 'bundle');
      return node.value;
    });
  }
  ...
  findBundlesWithDependency(dependency) {
    return this._graph.getNodeIdsConnectedTo((0, _nullthrows().default)(this._graph.getNodeIdByContentKey(dependency.id)), 'contains').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).filter(node => node.type === 'bundle').map(node => {
      (0, _assert().default)(node.type === 'bundle');
      return node.value;
    });
  }
  ...
  getDependencyResolution(dep, bundle) {
    ...
    if (resolved == null) {
      ...
      }, this._graph.getNodeIdByContentKey(dep.id), 'references');
    }
  ...
  isAssetReferencedByDependant(bundle, asset) {
    ...
    if (this._graph.getNodeIdsConnectedTo(assetNodeId, 'references').map(id => this._graph.getNode(id)).filter(node => (node === null || node === void 0 ? void 0 : node.type) === 'dependency' && node.value.priority === _types().Priority.lazy).length > 0) {
    ...
    let siblingBundles = new Set(this.getBundleGroupsContainingBundle(bundle).flatMap(bundleGroup => this.getBundlesInBundleGroup(bundleGroup))); // Check if any of this bundle's descendants, referencers, bundles referenced
  ...
  findReachableBundleWithAsset(bundle, asset) {
    ...
    for (let bundleGroup of bundleGroups) {
      ...
      let parentBundleNodes = this.getParentBundlesOfBundleGroup(bundleGroup).map(bundle => (0, _nullthrows().default)(this._graph.getNodeByContentKey(bundle.id))); // Find the nearest ancestor bundle that includes the asset.

      for (let bundleNode of parentBundleNodes) {
        (0, _assert().default)(bundleNode.type === 'bundle');

        this._graph.traverseAncestors(this._graph.getNodeIdByContentKey(bundleNode.id), (nodeId, ctx, actions) => {
          let node = (0, _nullthrows().default)(this._graph.getNode(nodeId));
  ...
  traverseBundles(visit, startBundle) {
    ...
    }, visit, startBundle ? this._graph.getNodeIdByContentKey(startBundle.id) : null, ['bundle', 'references']);
  }
  ...
  getDirectParentBundleGroups(bundle) {
    return this._graph.getNodeIdsConnectedTo((0, _nullthrows().default)(this._graph.getNodeIdByContentKey(bundle.id)), 'bundle').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).filter(node => node.type === 'bundle_group').map(node => {
      (0, _assert().default)(node.type === 'bundle_group');
      return node.value;
    });
  }
  ...
  getBundlesInBundleGroup(bundleGroup) {
    ...
    for (let bundleNodeId of this._graph.getNodeIdsConnectedFrom(this._graph.getNodeIdByContentKey((0, _utils2().getBundleGroupId)(bundleGroup)), 'bundle')) {
      let bundleNode = (0, _nullthrows().default)(this._graph.getNode(bundleNodeId));
      (0, _assert().default)(bundleNode.type === 'bundle');
  ...
  getContentHash(bundle) {
    ...
    this.traverseAssets(bundle, asset => {
      hash.writeString([this.getAssetPublicId(asset), asset.outputHash, asset.filePath, _querystring().default.stringify(asset.query), asset.type, asset.uniqueKey].join(':'));
    });
  ...
  isEntryBundleGroup(bundleGroup) {
    return this._graph.getNodeIdsConnectedTo((0, _nullthrows().default)(this._graph.getNodeIdByContentKey((0, _utils2().getBundleGroupId)(bundleGroup))), 'bundle').map(id => (0, _nullthrows().default)(this._graph.getNode(id))).some(n => n.type === 'root');
  }
