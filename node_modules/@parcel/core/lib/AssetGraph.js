...
function nodeFromAssetGroup(assetGroup) {
  var _assetGroup$code, _assetGroup$pipeline;

  return {
    id: (0, _hash().hashString)(assetGroup.filePath + assetGroup.env.id + String(assetGroup.isSource) + String(assetGroup.sideEffects) + ((_assetGroup$code = assetGroup.code) !== null && _assetGroup$code !== void 0 ? _assetGroup$code : '') + ':' + ((_assetGroup$pipeline = assetGroup.pipeline) !== null && _assetGroup$pipeline !== void 0 ? _assetGroup$pipeline : '') + ':' + (assetGroup.query ? JSON.stringify((0, _utils().objectSortedEntries)(assetGroup.query)) : '')),
    type: 'asset_group',
    value: assetGroup,
    usedSymbolsDownDirty: true
  };
...
class AssetGraph extends _ContentGraph().default {
  constructor(opts) {
    if (opts) {
      let {
        hash,
  ...
  resolveAssetGroup(assetGroup, assets, correspondingRequest) {
    ...
    for (let {
      ...
      assetNode = (0, _nullthrows().default)(this.getNodeByContentKey(assetNode.id));
