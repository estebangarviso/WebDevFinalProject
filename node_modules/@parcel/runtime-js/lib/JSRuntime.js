    ...
    for (let dependency of otherDependencies) {
      ...
      if (bundle.env.outputFormat === 'commonjs' && mainBundle.type === 'js') {
        assets.push({
          filePath: __filename,
          dependency,
          code: `module.exports = __parcel__require__("./" + ${getRelativePathExpr(bundle, mainBundle, options)})`,
          env: {
            sourceType: 'module'
          }
        });
...
function shouldUseRuntimeManifest(bundle, options) {
  let env = bundle.env;
  return !env.isLibrary && !bundle.isInline && env.outputFormat === 'global' && env.isBrowser() && options.mode === 'production';
}