...
class ScopeHoistingPackager {
  constructor(options, bundleGraph, bundle, parcelRequireName) {
    ...
    this.isAsyncBundle = this.bundleGraph.hasParentBundleOfType(this.bundle, 'js') && !this.bundle.env.isIsolated() && ((_this$bundle$getMainE = this.bundle.getMainEntry()) === null || _this$bundle$getMainE === void 0 ? void 0 : _this$bundle$getMainE.bundleBehavior) !== 'isolated';
  ...
  async package() {
    ...
    if (this.bundle.env.isLibrary || this.bundle.env.outputFormat === 'commonjs') {
      let bundles = this.bundleGraph.getReferencedBundles(this.bundle).filter(b => !b.isInline);
    ...
    if (this.bundle.env.outputFormat === 'global' && this.bundle.env.sourceType === 'script') {
      ...
      if (mapBuffer) {
        map = new (_sourceMap2().default)(mapBuffer);
      }
    ...
    this.bundle.traverse((node, shouldWrap) => {
      switch (node.type) {
        ...
        case 'asset':
          ...
          if (shouldWrap || node.value.meta.shouldWrap || this.isAsyncBundle || this.bundle.env.sourceType === 'script' || this.bundleGraph.isAssetReferencedByDependant(this.bundle, node.value)) {
  ...
  buildAsset(asset, code, map) {
    ...
    let sourceMap = this.bundle.env.sourceMap && map ? new (_sourceMap2().default)(map) : null; // If this asset is skipped, just add dependencies and not the asset's content.
...
`;
  ...
  buildAssetPrelude(asset, deps) {
    ...
    asset.symbols.hasExportSymbol('*') && [...usedSymbols].some(s => !asset.symbols.hasExportSymbol(s)); // If the asset doesn't have static exports, should wrap, the namespace is used,
  ...
  buildBundlePrelude() {
    ...
    if (this.needsPrelude) {
      ...
      let mightBeFirstJS = parentBundles.length === 0 || parentBundles.some(b => b.type !== 'js') || this.bundleGraph.getBundleGroupsContainingBundle(this.bundle).some(g => this.bundleGraph.isEntryBundleGroup(g)) || this.bundle.env.isIsolated() || ((_this$bundle$getMainE2 = this.bundle.getMainEntry()) === null || _this$bundle$getMainE2 === void 0 ? void 0 : _this$bundle$getMainE2.bundleBehavior) === 'isolated';
